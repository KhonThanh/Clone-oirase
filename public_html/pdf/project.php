...
<?php } else if ($lang=='en') {?>
For the conservation of nature of Oirase Stream,<span class="disp_ib"> to take your time slowly to beautiful clear Stream and Beech forest.</span>
<?php } else {?>
...
<p class="cap-01">
<?php if ($lang=='co'){?>
이를 위해, 「오이라세 계류」자연환경보전에 기여하는 활동을 하자는 취지로, 산업관광기관, 자연환경기관, 행정기관 등으로 조직된<a href="<?php echo $site_url;?>/pdf/member2022.pdf" target="_blank">「오이라세 계류 에코투어리즘 프로젝트 실행위원회」 ＜PDF:168KB＞</a>에서, 2008년부터「오이라세 계류 에코리즘 프로젝트」를 실행하게 되었습니다.
<?php } else if ($lang=='chi') {?>
为此，官民合力，携手推动保护“奥入濑溪流”自然环境之活动。产业观光、自然环境、行政各机关合作成立了<a href="<?php echo $site_url;?>/pdf/member2022.pdf" target="_blank">“奥入濑溪流生态旅游规划实行委员会”[PDF:168KB]</a>，并于2008年开始实行“奥入濑溪流生态旅游规划”。
<?php } else if ($lang=='tchi') {?>
為此，官民合力，攜手推動保護“奧入瀨溪流”自然環境之活動。產業觀光、自然環境、行政各機關合作成立了<a href="<?php echo $site_url;?>/pdf/member2022.pdf" target="_blank">“奧入瀨溪流生態旅遊規劃實行委員會”[PDF:168KB]</a>，並於2008年開始實行“奧入瀨溪流生態旅遊規劃”。
<?php } else if ($lang=='en') {?>
Therefore, let the govenrment and the people recommend the acitivites necessary to ensure the natural conservation of the [Oirase Stream].  Industrial tourism and natural environmental preservations were organized by the institution of administrative <a href="<?php echo $site_url;?>/pdf/member2022.pdf" target="_blank">"Oirase Stream  Eco-Tourism project Executive Committee" [PDF:168KB]</a> thus developing the Oirase Stream Eco Tourism project of 2008.
<?php } else {?>
このため、『奥入瀬渓流』の自然環境保全に資する活動を官民一体となって推進することを趣旨として、産業観光・自然環境・行政の各機関で組織する<a href="<?php echo $site_url;?>/pdf/member2022.pdf" target="_blank">「奥入瀬渓流エコツーリズムプロジェクト実行委員会」[PDF:168KB]</a>では、平成20年から『奥入瀬渓流エコツーリズムプロジェクト』を展開することとしました。
<?php } ?>
</p>
<br />
<p class="cap-01">
...
<p class="ovf_h">
<?php if ($lang=='co'){?>
오이라세계곡 승용차교통규제는, 「오이라세계류 이용적정화협의회」의 심의를 거쳐서 실시됩니다.
<?php } else if ($lang=='chi') {?>
奥入濑溪流私家车交通管制是经过“奥入濑溪流利用适正化协议会”的审议后而实施的。
<?php } else if ($lang=='tchi') {?>
奧入瀨溪流私家車交通管制是經過“奧入瀨溪流利用適正化協議會”的審議後而實施的。
<?php } else if ($lang=='en') {?>
The Oirase Stream private vehicle traffic restriction is carried out after deliberation with [The Oirase Stream Use Optimization Council].
<?php } else {?>
